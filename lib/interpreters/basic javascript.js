...
exports.interpret = function (file, content, options) {
	...
	try {
		...
		console.error("Error instrumenting file", file, options.verbose ? ex : ex.message);
	...
	var analyzing = [];
	// list of all lines' id encountered in this file
	var lines = [];
	// list of all conditions' id encountered in this file
	var allConditions = [];
	// list of all functions' id encountered in this file
	var allFunctions = [];
	...
	function countIf() {
		...
		if (self[0].start && analyzing.indexOf(self) < 0) {
			var decision = self[1];
			var lineId = self[0].name + "_" + self[0].start.line;
	 ...
	 * a function call.
	 * The condition id is generated from the lineId (@see countLine) plus the character
	...
	function wrapCondition(decision, lineId, parentPos) {
		...
		if (isSingleCondition(decision)) {
			if (!lineId) {
				// Conditionals might end up here
				lineId = getConditionLineId(decision);
			}

			var pos = getPositionStart(decision, parentPos);
			var endpos = getPositionEnd(decision, parentPos);
			var condId = [lineId, pos, endpos].join("_");
	...
	/**
	 * Whether or not the if decision has only one boolean condition
	 */
	...
	/**
	 * Get the line identifier of a given condition. This is build using the line name
	 * and position
	 */
	function getConditionLineId (condition) {
		if (condition[0] && condition[0].start) {
			return condition[0] + "_" + condition[0].start.line;
		} else if (condition[0] === "binary") {
			return getConditionLineId(condition[2]);
		} else if (condition[0] === "unary-prefix") {
			return getConditionLineId(condition[2]);
		} else {
			return "unk_0";
		}
	...
	function getPositionStart (decision, defaultValue) {
		...
		} else if (decision[0] === "unary-prefix") {
			// Unary prefixes are ++ -- typeof void
			// Uglify doesn't tell where they start, try to guess
			var space = ["typeof", "void", "delete"].indexOf(decision[1]) !== -1 ? 1 : 0;
			return getPositionStart(decision[2]) - decision[1].length - space;
		} else if (decision[0] === "binary") {
			return getPositionStart(decision[2]);
		} else {
	...
	function getPositionEnd (decision, defaultValue) {
		if (decision[0].end) {
			return decision[0].end.endpos;
		} else if (decision[0] === "unary-prefix") {
			return getPositionEnd(decision[2]);
		} else if (decision[0] === "binary") {
			return getPositionEnd(decision[3]);
		} else {
	 ...
	 * Instrumenting this code would return
	 *
	 ...
	 *    }
	 *
	 * Label can be wrapped by countLine, however the subsequent for shouldn't be wrapped.
	 *
	...
	function countFunction () {
		...
		if (this[0].start && analyzing.indexOf(this) < 0) {
			...
			if (options["function"]) {
				...
				rememberFunction(fnName + "_" + position);
			}
	...
	function wrapConditionals () {
		...
		if (self[0].start && analyzing.indexOf(self) < 0) {
			...
			// Here we might be in a conditional assignment
			self[1] = wrapCondition(self[1]);
		}
	};
