...
exports.format = function (content, error, asIs) {
	return {
		clientCode : asIs ? content : file.markInstrumented(content),
		error : !!error
	};
	};
...
exports.shouldBeExcluded = function (location, exclude) {
	...
	return !file.wanted(location, exclude);
};
 ...
 */
exports.isInstrumented = file.isInstrumented;
...
exports.generate = function (file, content, options, structure) {
	// Encode the original file in Base64 so it doesn't have to be encoded 
	// by the client when submitting the report (it contains unsafe characters)
	var codeStructure = highlight(content, structure);
	var originalCode = new Buffer(JSON.stringify(codeStructure)).toString('base64');
 ...
 * @param {Object} args Object containing list of statements, coditions, ...
 */
