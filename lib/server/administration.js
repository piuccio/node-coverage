...
exports.createApp = function (options) {
	...
	app.get("/json/:report", function (req, res) {
		if (req.params.report === "all") {
			reportsList(function (err, reports) {
				if (err) {
					res.send("Error while reading reports list\n" + err, 500);
				} else {
					res.json({
						reports : reports,
						conf : {
							adminRoot : adminRoot,
							reportFolder : docRoot,
							o : options
						}
					});
				}
			...
			res.send("Unknown action " + req.params.report, 404);
		}
	});

	app.get("/json/report/:id", function (req, res) {
		readReport(req.params.id, sendReport.bind(this, req, res));
	});
	app.get("/json/report/:id/sort/:what/:how", function (req, res) {
		readReport(req.params.id, sendReport.bind(this, req, res));
	});
