 ...
 *        covered : number of executed statement,
...
function organizeFunctionCallsByFile (coveredFunctions) {
	...
	coveredFunctions.forEach(function (fnDescriptor) {
		...
		var fnName = fnDescriptor[1] + "_" + fnDescriptor[2];
		if (!byFile[file]) {
			byFile[file] = {};
		}
 ...
 *  highlight : source code with additional markup wrapping non covered statements / conditions,
 *  statements
 *     total : total number of lines,
...
function generate (file, container) {
	...
	report.highlight = highlight(report);
	return report;
};
 ...
 * @return {Object} a single report @see generateAll
 */
...
exports.stats = require("./statistics").getStats;