 ...
 */
var instrument = function (file, content, options) {
	options = options || {
		"function" : true,
		"condition" : true
	};
	...
	function countFunction () {
		...
		if (this[0].start && analyzing.indexOf(this) < 0) {
			...
			if (options["function"]) {
				body.splice(0, 0, [ "stat",
					[ "call", 
						...
						[
							["string", file],
							["string", fnName],
							["string", "" + this[0].start.line + "_" + this[0].start.pos]
						]
					]
				]);
				...
				rememberFunction(fnName + this[0].start.line + "_" + this[0].start.pos);
			}
	...
	function giveNameToAnonymousFunction () {
		...
		if (node[0].name == "var" || node[0].name == "object") {
			node[1].forEach(function (assignemt) {
				if (assignemt[1] && assignemt[1][0].name === "function") {
			...
			if (node[3][0].name === "function") {
				if (node[2][0].name === "name") {
					node[3][0].anonymousName = node[2][1];
				} else if (node[2][0].name === "dot") {
					node[3][0].anonymousName = node[2][2];
				}
			}
		}
	};
	...
	return clientCode.generate(file, lines, allConditions, allFunctions, code, options.doHighlight);
};
